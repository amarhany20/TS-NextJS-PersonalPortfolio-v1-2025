...
|--------|-------------|-------|
| PostgreSQL | Local, staging, and production | Current schema and bootstrap path are aligned around `DATABASE_URL` and Prisma migrations. |

The active onboarding path is env/bootstrap-driven. The old setup wizard is not the supported
runtime path for launch.
...
- **Future extension:** If a guided setup flow is ever restored, it should be rebuilt around the
  env/bootstrap model instead of reintroducing parallel onboarding paths.
