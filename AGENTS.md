...
- Always use the isomorphic logger (`@/utils/logger` or `@/server/server-utils/logger`) for operational logging, error handling, API telemetry, and service exceptions instead of raw `console.log` / `console.error`.
...
| 4.3.0   | 2026-07-30 | GitHub Copilot | Removed `src/app/global-error.tsx` to fix a pre-existing Next.js 16 pre-rendering bug (`useContext` null during `/_global-error` static generation). Added `htmlFor`/`id` attributes to blog editor form labels for accessibility and Playwright `getByLabel` compatibility. Build passes against real Neon PostgreSQL. Isolated Playwright e2e: 21/21 pass.                                                                                                                                                                                                                                       |
