  ...
  test('creates, publishes, edits, and deletes a recommendation', async ({ request }) => {
    ...
    try {
      ...
      await expect
        ...
        .toBe('Automation smoke coverage for recommendations (edited).');
...
async function publishRecommendation(request: APIRequestContext, id: string) {
  const response = await request.patch(`/api/v1/recommendations/${id}`, {
    data: { published: true },
  });
  const responseText = response.ok() ? '' : await response.text();
  expect(response.ok(), responseText).toBeTruthy();
...
async function cleanupRecommendation(name: string, request: APIRequestContext) {
  ...
  const recommendations = (payload?.data?.recommendations ?? []) as Array<{
    id: string;
    name: string;
  }>;
  ...
  const deleteResponse = await request
    .delete(`/api/v1/recommendations/${match.id}`)
    .catch(() => null);
  ...
  console.warn(
    `Cleanup failed for recommendation ${match.id}: ${deleteResponse.status()} ${responseText}`,
  );
}
