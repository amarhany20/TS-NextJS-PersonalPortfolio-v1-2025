		...
		<section id="certificates" className="scroll-mt-8">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					{certificates.map((cert) => {
						if (cert.verifyUrl) {
							return (
								<a key={cert.id} href={cert.verifyUrl} className="group" aria-label={`Open ${cert.name} document`}>
									<SectionCard hover className="group">
										...
										<div className="mb-4">
											{cert.description ? <p className="text-[var(--text-secondary)] text-sm mb-2">{cert.description}</p> : null}
