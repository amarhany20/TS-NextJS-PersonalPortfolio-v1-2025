...
interface ExperienceItemProps { exp: any }

function ExperienceItem({ exp }: ExperienceItemProps) {
	const [open, setOpen] = useState(false);

	const truncatedDescription = exp.description?.length > 180
		? exp.description.slice(0, 180) + '…'
		: exp.description;
			...
			<div className="relative z-10 pl-5 sm:pl-6">
				...
				<div className="flex flex-col gap-4 sm:gap-3 md:flex-row md:items-start md:justify-between mb-4 md:mb-5">
					<div className="flex-1 min-w-0">
						...
						<div className="flex flex-wrap gap-x-4 gap-y-1 text-[var(--text-secondary)] text-xs md:text-sm">
							...
							{exp.isCurrent && (
								<span className="px-2 py-0.5 rounded-full bg-[var(--accent-primary)]/15 text-[var(--accent-primary)] text-[10px] font-semibold uppercase tracking-wide">Current</span>
							)}
						</div>
					</div>
								<div className="flex items-center justify-end gap-3">
									<span className={`px-2 py-0.5 rounded-full text-[11px] font-medium tracking-wide ${exp.type === 'Current' ? 'bg-[var(--accent-primary)] text-black' : 'bg-[var(--accent-muted)] text-[var(--text-secondary)]'}`}>{exp.type}</span>
									<button
										onClick={() => setOpen(o => !o)}
										aria-expanded={open}
				...
				<div className="mb-3 md:mb-4">
					...
					<p className="text-[var(--text-secondary)] text-sm leading-relaxed line-clamp-4 md:line-clamp-none">
						{open ? exp.description : truncatedDescription}
					</p>
				</div>
