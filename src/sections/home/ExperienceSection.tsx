    ...
    return (
      <section id="experience" className="scroll-mt-8">
        ...
        <div className="flex items-center justify-center py-12">
          <p className="text-[var(--text-secondary)]">No experience data</p>
        </div>
      </section>
    );
  return (
...
function ExperienceItem({ exp }: ExperienceItemProps) {
  ...
  const truncatedDescription =
    exp.impact.length > 180 ? exp.impact.slice(0, 180) + '…' : exp.impact;

  return (
    <SectionCard
      hover
      className={`group relative overflow-hidden transition-colors ${open ? 'ring-1 ring-[var(--accent-primary)]/30' : ''}`}
    >
      ...
      <div className="relative z-10 pl-5 sm:pl-6">
        ...
        <div className="flex flex-col gap-4 sm:gap-3 md:flex-row md:items-start md:justify-between mb-4 md:mb-5">
          ...
          <div className="flex items-center justify-end gap-3">
            <span
              className={`px-2 py-0.5 rounded-full text-[11px] font-medium tracking-wide ${exp.present ? 'bg-[var(--accent-primary)] text-black' : 'bg-[var(--accent-muted)] text-[var(--text-secondary)]'}`}
            >
              {exp.present ? 'Current' : 'Past'}
            </span>
            <button
              onClick={() => setOpen((o) => !o)}
              aria-expanded={open}
        ...
        {/* Collapsible Details */}
        <div
          className={`grid transition-[grid-template-rows] duration-300 ease-in-out ${open ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'} overflow-hidden`}
