...
export default function SummarySection({ hero }: SummarySectionProps) {
	...
	return (
		<section id="summary" className="relative flex flex-col items-start gap-6 py-8 md:py-14 scroll-mt-8">
			<h1 className="text-3xl sm:text-5xl md:text-6xl font-bold text-[var(--accent-primary)] drop-shadow-lg">{hero.greeting}</h1>
			{hero.subtitle ? <h2 className="text-xl sm:text-2xl md:text-3xl font-semibold text-[var(--accent-secondary)]">{hero.subtitle}</h2> : null}
			{descriptionHtml ? (
				<div className="max-w-2xl text-[var(--text-secondary)] text-lg mb-2 prose prose-invert" dangerouslySetInnerHTML={{ __html: descriptionHtml }} />
			) : null}
			<div className="flex flex-wrap gap-4 mt-2">
				{hero.primaryButton ? (
					...
					>
						{hero.primaryButton.text?.trim() || 'Get in Touch'} <ArrowRight className="ml-2" size={20} />
					</a>
				) : null}
				<a
