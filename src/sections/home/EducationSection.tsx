		...
		<section id="education" className="scroll-mt-8">
			...
			<div className="space-y-8">
				{education.map((edu, index) => (
					<SectionCard key={(edu.id ?? index).toString()} hover className="group relative overflow-hidden">
							...
							<div className="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-6">
								<div className="flex-1">
									...
									<div className="flex flex-wrap items-center gap-4 text-[var(--text-secondary)] text-sm mb-2">
										{edu.location ? <div className="flex items-center gap-1"><MapPin size={16} />{edu.location}</div> : null}
										<div className="flex items-center gap-1"><Calendar size={16} />{edu.end ? `${edu.start} - ${edu.end}` : `${edu.start} - Present`}</div>
									</div>
									{edu.gpa ? (
