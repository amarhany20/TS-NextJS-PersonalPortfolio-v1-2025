...
export default function ContactSection({ details, socialLinks }: ContactSectionProps) {
  ...
  const whatsappDisplay =
    phones[0]?.e164?.replace(/^(\+\d{1,3})(\d{3})(\d{3})(\d{4})$/, '$1 $2 $3 $4') ||
    phones[0]?.e164;
  const location = details.location || 'Remote';
  const leftLead =
    details.leads?.left ??
    'Direct channels for professional inquiries, consulting engagements, collaborations, or technical leadership support.';
  const rightLead =
    details.leads?.right ?? 'Ready to start? Reach out via email, WhatsApp, or LinkedIn.';

  const filteredSocialLinks = (socialLinks ?? []).filter(
    (link) => !/website|portfolio/i.test(link.label),
  );
      ...
      <div className="relative grid grid-cols-1 xl:grid-cols-3 gap-12 items-start">
        <div className="space-y-8 z-10 xl:col-span-2">
          <header className="space-y-4">
            ...
            <p className="text-[var(--text-secondary)] max-w-prose text-sm md:text-base">
              {contactDescription}
            </p>
          </header>
          <p className="text-[var(--text-secondary)] leading-relaxed max-w-prose text-sm md:text-base">
            {leftLead}
          </p>
          ...
          <div className="flex flex-col gap-6">
            {primaryEmail ? (
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-[var(--text-secondary)] text-base uppercase tracking-wide mb-1">
                    Email
                  </p>
                  <a
                    href={`mailto:${primaryEmail}`}
                    className="text-foreground font-medium hover:text-[var(--accent-primary)] transition-colors"
                  >
                    {primaryEmail}
                  </a>
                </div>
              ...
              {filteredSocialLinks.map((link) => {
                ...
                return (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-3 py-2 rounded-md bg-[var(--accent-muted)] hover:bg-[var(--accent-primary)] hover:text-black transition-colors shadow-sm"
                  >
