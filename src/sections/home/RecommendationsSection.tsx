    ...
    <section id="recommendations" className="scroll-mt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {recommendations.map((rec: Recommendation, index) => {
          ...
          const multipleLinks =
            hasLetter && hasLinkedIn && rec.recommendationLetterUrl !== rec.linkedin;
          ...
          const cardInner = (
            <SectionCard hover className="group h-full">
              ...
              <blockquote className="text-[var(--text-secondary)] text-base mb-5 leading-relaxed">
                &ldquo;{rec.content.length > 180 ? rec.content.slice(0, 180) + '…' : rec.content}
                &rdquo;
              </blockquote>
              {rec.content.length > 180 && (
                <div className="text-[var(--accent-primary)] text-sm font-medium hover:underline">
                  Read full
                </div>
              )}
