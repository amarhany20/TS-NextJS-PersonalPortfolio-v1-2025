...
export default function SkillsSection({ groups }: SkillsSectionProps) {
  const ordered = (groups ?? [])
    .slice()
    .sort((a, b) => SORT_ORDER.indexOf(a.id) - SORT_ORDER.indexOf(b.id));
  return (
  ...
  return (
    <SectionCard className="flex flex-col shadow-sm hover:shadow-md transition-shadow">
      <div className="mb-4 pr-4">
        <h3 className="text-base md:text-lg font-semibold tracking-wide uppercase text-[var(--text-secondary)]">
          {group.title}
        </h3>
        {group.summary && (
          <p className="mt-1 text-sm md:text-sm leading-relaxed text-[var(--text-muted)]">
            {group.summary}
          </p>
        )}
      </div>
      <ul className="flex flex-wrap mt-auto">
        {group.skills.map((s) => (
          <li
            key={s.name}
            className="mr-1 mb-1 rounded-md bg-[var(--panel)] px-3 py-2 text-sm font-medium text-[var(--foreground)]"
          >
            {s.name}
          </li>
        ))}
