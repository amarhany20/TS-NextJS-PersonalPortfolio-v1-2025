        ...
        {recommendations.map((rec, index) => (
          <SectionCard key={index} hover className="group">
            ...
            <div className="flex items-start gap-3">
              {rec.photo && <Image src={rec.photo} alt={rec.name} width={40} height={40} className="w-10 h-10 rounded-full object-cover" />}
              <div className="flex-1">
                <h4 className="font-semibold text-foreground text-sm">{rec.name}</h4>
                <p className="text-[var(--accent-primary)] text-xs font-medium">{rec.position}</p>
