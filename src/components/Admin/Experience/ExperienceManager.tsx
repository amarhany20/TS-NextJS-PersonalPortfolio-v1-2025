...
export function ExperienceManager({ initialExperience }: ExperienceManagerProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return items
      ...
      .filter((item) => {
        if (!term) return true;
        return [item.company, item.title, item.location, item.impact].some((field) =>
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const description = payload?.error?.message ?? 'Unable to refresh experience entries.';
        throw new Error(description);
      }

      const nextItems: Experience[] = payload?.data?.experience ?? [];
      setItems(nextItems);
      showToast({ variant: 'success', title: 'Experience refreshed' });
      router.refresh();
    } catch (error) {
  ...
  const handleTogglePublish = async (record: Experience) => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => null);
      if (!response.ok) {
        const description = payload?.error?.message ?? 'Unable to update entry.';
        throw new Error(description);
      }
  ...
  const handleDelete = async (record: Experience) => {
    ...
    setBusyId(record.id);
    try {
      const response = await fetch(`/api/v1/experience/${record.id}`, { method: 'DELETE' });
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        const description = payload?.error?.message ?? 'Unable to delete entry.';
        throw new Error(description);
      }
    ...
    <section className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <input
          ...
          placeholder="Search by company, title, or impact"
                ...
                <tr key={item.id} className="border-t border-[var(--border)]/60">
                  ...
                  <td className="px-4 py-4 align-top">
                    <div className="flex items-center justify-end gap-2 text-xs">
                      <button
                        type="button"
                        aria-label={`${item.published ? 'Unpublish' : 'Publish'} ${item.company}`}
