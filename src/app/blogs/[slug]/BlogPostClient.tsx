    ...
    <article className="space-y-8">
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap gap-2">
            {post.categories.map((category) => (
              <span key={category.id} className="px-3 py-1 rounded-full bg-[var(--accent-muted)] text-xs font-medium">
                {category.name}
              </span>
            ))}
          </div>
          <h1 className="text-4xl font-semibold text-foreground">{post.title}</h1>
          {post.summary && <p className="text-lg text-[var(--text-secondary)] max-w-3xl">{post.summary}</p>}
      ...
      {post.tags.length > 0 && (
        <footer className="flex flex-wrap gap-2 text-xs text-[var(--text-secondary)]">
          {post.tags.map((tag) => (
            <span key={tag.id} className="px-2 py-1 rounded-full border border-[var(--border)]">#{tag.name}</span>
          ))}
        </footer>
      )}
    </article>
...
function buildContentMarkup(content: string) {
  ...
  const paragraphs = trimmed
    .split(/\n{2,}/)
    .map((paragraph) => DOMPurify.sanitize(paragraph, { ALLOWED_TAGS: [], ALLOWED_ATTR: [] }).trim())
    .filter(Boolean)
    .map((paragraph) => `<p>${paragraph}</p>`)
    .join('');

  return paragraphs || `<p>${DOMPurify.sanitize(trimmed, { ALLOWED_TAGS: [], ALLOWED_ATTR: [] })}</p>`;
}
