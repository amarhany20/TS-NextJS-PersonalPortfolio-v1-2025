...
let DashboardService: typeof import('../DashboardService')['DashboardService'];
...
describe('DashboardService', () => {
  beforeEach(async () => {
    ...
    ({ DashboardService } = await import('../DashboardService'));
  });
  ...
  it('flags pending setup and missing env vars', async () => {
    ...
    expect(overview.meta.pendingSetup).toBe(true);
    expect(overview.meta.missingEnvVars).toContain('NEXT_PUBLIC_SITE_URL');
    expect(overview.quickLinks[0].title).toContain('Finish setup');
